...
from quantum_walk import quantum_centrality_scores
...
def run_for_window(returns, window_days):
    ...
    try:
        raw_scores = quantum_centrality_scores(ret_window, time=config.EVOLUTION_TIME, weighted=config.USE_WEIGHTED_GRAPH)
    except Exception as e:
        print(f"    Error: {e}")
        return None
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "quantum_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "weighted_graph": config.USE_WEIGHTED_GRAPH,
        "universes": {}
    }
    ...
    out_file = f"output/quantum_walk_{timestamp}.json"
